        ...
        stage('DeployToProduction') {
            steps {
                withCredentials([usernamePassword(credentialsId: "e956abb7-90da-440a-8bd1-8d16c2435495", usernameVariable: "bristlbeak_jenkins", passwordVariable: "rootroot")]) {
                    script {
                        sh "sshpass -p 'rootroot' ssh -o StrictHostKeyChecking=no bristlbeak_jenkins@172.17.0.1 \'sudo docker pull slobodyanyuk/jenkins_voting_app:${env.BUILD_NUMBER}'"
                        try {
                            sh "sshpass -p 'rootroot' ssh -o StrictHostKeyChecking=no bristlbeak_jenkins@172.17.0.1 \"sudo docker stop jenkins_voting_app\""
                            sh "sshpass -p 'rootroot' ssh -o StrictHostKeyChecking=no bristlbeak_jenkins@172.17.0.1 \"sudo docker rm jenkins_voting_app\""
                        } catch (err) {
