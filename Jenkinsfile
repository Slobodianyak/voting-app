        ...
        stage('DeployToProduction') {
            steps {
                withCredentials([usernamePassword(credentialsId: "e956abb7-90da-440a-8bd1-8d16c2435495", usernameVariable: "bristlbeak_jenkins", passwordVariable: "rootroot")]) {
                    script {
                        sh "sudo sshpass -S 'rootroot' ssh -o StrictHostKeyChecking=no bristlbeak_jenkins@172.17.0.1 \'sudo docker pull bristlbeak_jenkins@172.17.0.1:${env.BUILD_NUMBER}'"
                        try {
                        ...
                        sh "sudo sshpass -S 'rootroot' ssh -o StrictHostKeyChecking=no bristlbeak_jenkins@172.17.0.1 \"sudo docker run --restart always --name jenkins_voting_app -p 8080:8080 -d bristlbeak_jenkins@172.17.0.1:${env.BUILD_NUMBER}\""
                    }
                }
            }
        }    
